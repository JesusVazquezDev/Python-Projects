...
# generate a private key -- g^x mod p
x = ec.generate_private_key(EC_CURVE)
gx = x.public_key()
gx_bytes = gx.public_bytes(ENCODING, FORMAT)

# send to server and get gy
r = requests.get(URL + '/dh', params={
      'gx': base64.urlsafe_b64encode(gx_bytes)
})
jsonResponse = r.json()
c = jsonResponse['c']
gy_response = jsonResponse['gy']
gy_bytes = base64.urlsafe_b64decode(gy_response)
gy = ec.EllipticCurvePublicKey.from_encoded_point(EC_CURVE, gy_bytes)

# figure out what gxy is
